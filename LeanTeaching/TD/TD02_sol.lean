...
lemma add_zero' (n : N) : n + zero = n := by
  rfl
  done
...
lemma add_assoc : a + b + c = a + (b + c) := by
  ...
  | succ c hrec =>
    rw [add_succ, add_succ, add_succ, hrec]
...
lemma one_mul : 1 * a = a := by
  ...
  | zero => rfl
  | succ n hn =>
    rw [mul_succ, hn, one_eq_succ_zero, add_succ, add_zero]
  done

lemma mul_add : a * (b + c) = a * b + a * c := by
  induction b with
  | zero =>
    change a * (0 + c) = a * 0 + a * c
    rw [zero_add, mul_zero, zero_add]
  | succ n hn =>
    rw [succ_add, mul_succ, hn, mul_succ, add_assoc, add_comm (a*c), add_assoc]
  done
...
lemma mul_assoc : a * b * c = a * (b * c) := by
  ...
  | zero =>
    rfl
  | succ n hn =>
    rw [mul_succ, hn, mul_succ, mul_add]
  done
...
lemma succ_mul : succ a * b = a * b + b := by
  ...
  | zero => rfl
  | succ n hn =>
    rw [mul_succ, mul_succ, hn, add_assoc, add_assoc, add_succ,
      add_succ, add_succ, add_succ, add_comm n a]
...
lemma add_mul : (a + b) * c = a * c + b * c := by
  ...
  | zero => rfl
  | succ n hn =>
    rw [mul_succ, mul_succ, hn, mul_succ, add_assoc, add_assoc]
    congr 1
    rw [← add_assoc, add_comm _ a, add_assoc]
